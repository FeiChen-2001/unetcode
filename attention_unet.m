...
save('attention_unet_soil_model.mat', 'net'); % 修改保存的模型名称
...
function [lgraph, attOutName] = attentionGate(lgraph, xName, gName, F_g, F_l, blockName)
    ...
    mulLayer = functionLayer(@(X,Y) X .* Y, 'NumInputs', 2, 'Name', mulName); % 修正后的写法
